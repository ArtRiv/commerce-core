...
export class ProductsController {
  ...
  @ApiBadRequest(
    '`priceCents` is not an integer above zero, a category id is not a UUID, an image is not a URL, or the slug is not lowercase-hyphenated.',
  )
  ...
  @ApiOperation({
    ...
    description:
      'An inventory correction — "the shelf holds N" — not a delta. Selling is the other path: checkout decrements atomically inside its transaction, and nothing here is involved in that.\n\nBe aware this is last-write-wins against a concurrent sale, which is accepted for v1 (docs/specs/catalog.md).',
  })
